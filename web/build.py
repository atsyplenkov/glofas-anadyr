                ...
                if nc_files:
                    ...
                    for nc_file in nc_files:
                        ...
                        if not extracted.empty:
                            ...
                            print(
                                f"  Date range: {extracted['datetime'].min().date()} to {extracted['datetime'].max().date()}"
                            )
                            before_filter = len(extracted)
                            extracted = extracted[
                                extracted["datetime"].dt.date > last_data_date
                            ]
                            print(
                                f"  After filtering (>{last_data_date}): {len(extracted)} records (removed {before_filter - len(extracted)})"
                            )
                            for gauge_id in GAUGE_IDS:
                                gauge_data = extracted[
                                    extracted["gauge_id"] == gauge_id
                                ]
                    ...
                    if not all_raw_data:
                        ...
                        for gauge_id in GAUGE_IDS:
                            if gauge_id in all_raw_data:
                                ...
                                print(
                                    f"  Processing gauge {gauge_id}: {len(raw_df)} records"
                                )
                                ...
                                if not corrected_df.empty:
                                    ...
                                    merged_df = raw_df.merge(
                                        corrected_df, on="date", how="left"
                                    )
                                    merged_df["gauge_id"] = gauge_id
                                    merged_df["q_obs"] = None
                                    merged_df = merged_df[
                                        ["date", "gauge_id", "q_obs", "q_raw", "q_cor"]
                                    ]

                                    print(
                                        f"  Uploading {len(merged_df)} records for gauge {gauge_id}"
                                    )
                                    upload_incremental_to_s3(
                                        bucket,
                                        s3_client,
                                    ...
                                    print(
                                        f"  Warning: No corrected data for gauge {gauge_id}"
                                    )

                        if successful_uploads > 0:
                            update_s3_metadata(bucket, s3_client, update_date)
                            print(
                                f"Updated S3 with new data (update_date={update_date}, gauges_uploaded={successful_uploads})"
                            )
                        else:
                            print(
                                "Skipping metadata update: no gauges successfully uploaded"
                            )
        else:
            print(
                f"No missing years detected (last data: {last_data_date}, today: {today}, days diff: {days_since_last})"
            )
