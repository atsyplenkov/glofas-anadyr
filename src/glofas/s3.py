...
def get_s3_client(endpoint_url: Optional[str] = None):
    ...
    return boto3.client(
        "s3", endpoint_url=endpoint_url or os.environ.get("ENDPOINT_URL")
    )
...
def update_s3_metadata(
    bucket: str, s3_client, update_date: str, gauges: List[int] = GAUGE_IDS
):
    """Update metadata.json in S3."""
    metadata = {"last_update": update_date, "gauges": gauges}
...
def upload_incremental_to_s3(
    bucket: str, s3_client, gauge_id: int, new_data: pd.DataFrame, update_date: str
):
    ...
    try:
        ...
        combined_df = combined_df[
            ["date", "gauge_id", "q_obs", "q_raw", "q_cor", "update_date"]
        ]
