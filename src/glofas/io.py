...
def load_gauge_data_all(gauge_id: int, data_dir: Path = DATA_DIR / "hydro") -> pd.DataFrame:
    ...
    if "q_obs" not in df.columns: df["q_obs"] = None
    if "q_raw" not in df.columns: df["q_raw"] = None
    if "q_cor" not in df.columns: df["q_cor"] = None
...
def load_cv_results(cv_results_path: Path) -> dict:
    ...
    from .process import extract_kge # Avoiding circular import if possible, but extracting logic is better
